...
const App: React.FC = () => {
  ...
  const categories: string[] = (Array.from(new Set(predictions.map(p => p.category))) as string[]).sort();
      ...
      <main className="max-w-7xl mx-auto px-6">
        <div className="sticky top-4 z-50 mb-12">
          <div className="glass p-2 rounded-3xl shadow-2xl flex flex-col md:flex-row gap-4 items-center border-white/5">
            ...
            {view === 'Methodology' && (
              <div className="flex-grow flex items-center px-4 text-xs font-bold text-slate-500 uppercase tracking-widest">
                Benchmarking Framework & Process
              </div>
            )}
          ...
          <>
            ...
            {filteredPredictions.length === 0 && (
              <div className="py-32 text-center glass rounded-3xl border-white/5">
                ...
                <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">No records matching criteria</p>
                <button 
                  onClick={() => { setActiveScope('All'); setActiveCategory('All'); setActiveModel('All'); setSearchQuery(''); }}
                  className="mt-6 px-6 py-2 bg-blue-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-500 transition-all"
                >
                  Clear All Filters
                </button>
              </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
          <div>
            ...
            <p className="text-sm text-slate-500 leading-relaxed font-medium mb-6">
              An independent evaluation project tracking the forecasting accuracy of artificial intelligence models in real-world geopolitical and economic contexts.
            </p>
            <div className="space-y-3">
              <div className="p-4 rounded-xl bg-orange-500/5 border border-orange-500/20">
                 <div className="flex items-center gap-2 text-[10px] font-black text-orange-400 uppercase tracking-widest mb-1">
                 ...
                 <a 
                   ...
                   className="text-xs font-bold text-slate-400 hover:text-white transition-colors underline decoration-orange-500/30 underline-offset-4"
                 >
                   LLM powered by Adapta API Platform
                 </a>
              </div>
          ...
          <div className="flex flex-col gap-4">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Navigation</h4>
            <button onClick={() => setView('Methodology')} className="text-left text-sm font-bold text-slate-500 hover:text-blue-400 transition-colors">Methodology 2026</button>
            <button onClick={() => setView('Analytics')} className="text-left text-sm font-bold text-slate-500 hover:text-blue-400 transition-colors">Analytics</button>
            <button onClick={() => setView('Grid')} className="text-left text-sm font-bold text-slate-500 hover:text-blue-400 transition-colors">Predictions Grid</button>
          </div>
          <div className="flex flex-col gap-4">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Connect</h4>
            <div className="flex gap-4">
              ...
              <div 
                className="flex-grow glass p-3 rounded-xl text-center text-[10px] font-black uppercase tracking-widest text-slate-500 border-white/5 cursor-pointer hover:bg-white/10 transition-colors select-none"
                onClick={() => setShowAdminLogin(true)}
              >
                v1.2.1-ADAPTA {isAdmin && <span className="text-purple-400 ml-1">ADMIN</span>}
              </div>
            </div>
          </div>
        </div>
